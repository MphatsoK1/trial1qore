...
def generate_ai_question(difficulty, age, topic, question_number=1):
    ...
    question_types = {
        'easy': ['Malawian riddles'],
        'medium': ['African riddles'],
        'hard': ['Global riddles']
    }
...
{{
    ...
    try:
        logger.info(f"Generating AI riddle {question_number} for {age}y/o, {difficulty}, {topic}")
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": "You are a fun tutor who makes learning exciting by creating engaging educational riddles for children. Always respond with valid JSON. Create unique questions each time."
                },
                {
                    "role": "user", 
                    "content": prompt
        ...
        if all(key in result for key in required_keys):
            if len(result['options']) == 4 and result['correct'] in ['A', 'B', 'C', 'D']:
                logger.info(f"AI riddle {question_number} generated successfully")
                return result
            else:
        ...
        logger.error(f"AI riddle generation failed for riddle {question_number}: {e}")
